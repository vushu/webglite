...
(defn init-shader-program [vs-source fs-source]
  (let [vertex-shader (compile-shader c/VERTEX_SHADER vs-source)
...
(def program (init-shader-program vs-source fs-source))
(def attrib-locations {:vertex-position (gl/get-attrib-location ctx program "aVertexPosition")})
